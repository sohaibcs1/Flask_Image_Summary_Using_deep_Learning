...
from tensorflow.keras.models import load_model, Model                      # To load the pretrained model and to define a modified ResNet50 architecture for image encoding
from tensorflow.keras.preprocessing.text import Tokenizer                  # To load vocabulary for generating words
from tensorflow.keras.applications.resnet import ResNet50                  # For image encoding
from tensorflow.keras.applications.resnet50 import preprocess_input        # To transform image into the format expected by ResNet50 architecture (Size, Shape, Normalization etc.)
from tensorflow.keras.preprocessing.image import load_img, img_to_array    # To load image into memory and to convert image to numpy array
from tensorflow.keras.preprocessing.sequence import pad_sequences          # To generate sequences of a fixed length (pad the sequence if there's less words with enough fillers)
...
class utils:
    ...
    def get_image_features(img_abs_path):
        ...
        '''
        model = ResNet50(weights = "imagenet", include_top = False, pooling = "avg")                                                            # Instantiate a ResNet50 model
        model.layers.pop()   
                                                                 # Remove the last layer of pretrained ImageNet data
        ...
        print("img.shape befor reshaping: ", img.shape)
        img = img.reshape((1, rows, columns, channels))  
        print("img.shape before preprocessing: ", img.shape)                             # Redefine the image dimensions in a batch format
        img = preprocess_input(img)
        print("*********check2********")
        print("img shape after preprocessing: ", img.shape)
                                                           # Preprocess the input in order to bring it in a similar format as the imagenet preprocessing steps in original ResNet architecture
        features = Image_Feature_Generator.predict(img)
        print("features shape in function: ", features.shape)
